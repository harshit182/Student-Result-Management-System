...
title_label = tk.Label(win, text="Student Result Management System", font=("Arial", 30, "bold"), border=12, relief=tk.GROOVE, bg="lightgrey")
...
def fetch_data():
    ...
    # Fetch new data from the database and insert it into the table
    conn = pymysql.connect(host="localhost", user="root", password="", database="srms1")
    curr = conn.cursor()
...
def add_func():
    if rollno.get()== "" or class_var.get()=="":
        messagebox.showerror("Error1","Please fill all the fields!")
    else:
        try:
            ...
            curr.execute("INSERT INTO data VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s)", 
                         (rollno.get(), name.get(), class_var.get(), section.get(), 
                          contact.get(), fathersnm.get(), address.get(), gender.get(), dob.get()))
...
def update_func():
    try:
        conn = pymysql.connect(host="localhost", user="root", password="", database="srms1")
        curr = conn.cursor()
        query = "UPDATE data SET name = %s, class = %s, section = %s, contact = %s, fathersnm = %s, address = %s, gender = %s, dob = %s WHERE <correct_column_name> = %s"
        curr.execute(query, (name.get(), class_var.get(), section.get(), contact.get(), fathersnm.get(), address.get(), gender.get(), dob.get(), rollno.get()))
        conn.commit()
        conn.close()
        fetch_data()  # Refresh the table after updating
...
btn_frame = tk.Frame(detail_frame, bd=10, bg="lightgrey", relief=tk.GROOVE)
...
search_lbl = tk.Label(search_frame, text="Search ", font=("Arial", 14), bg="lightgrey")
...
search_in['value'] = ("Name", "Rollno", "Contact", "Father's Name", "Class", "Section", "D.O.B")
...
# Configure yscrollcommand option to link it with y_scroll scrollbar
student_table['yscrollcommand'] = y_scroll.set

# Configure xscrollcommand option to link it with x_scroll scrollbar
student_table['xscrollcommand'] = x_scroll.set

# Set the yscrollcommand option of the scrollbar to the yview method of the Treeview widget
y_scroll.config(command=student_table.yview)

# Set the xscrollcommand option of the scrollbar to the xview method of the Treeview widget
...
# Call fetch_data to populate the table initially
fetch_data()
